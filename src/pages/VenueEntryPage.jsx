...
const MEMBERS_COLLECTION = "clubRepresentatives";
const PLAYERS_COLLECTION = "participatingClubs";
const PLAYER_PHOTOS_COLLECTION = "participantLogos";
const WITHDRAWAL_REQUESTS_COLLECTION = "participationWithdrawalRequests";

const DEFAULT_CLUB_ID = "missing-venue";
const DEFAULT_CLUB_NAME = "League venue";
...
function clubRootDocRef(clubId) {
  return doc(db, "leagueVenues", safeClubIdFromSelectedClub({ id: clubId }));
}

function clubCollectionRef(clubId, collectionName) {
  return collection(db, "leagueVenues", safeClubIdFromSelectedClub({ id: clubId }), collectionName);
}

function clubDocRef(clubId, collectionName, docId) {
  return doc(db, "leagueVenues", safeClubIdFromSelectedClub({ id: clubId }), collectionName, String(docId || "").trim());
}
...
import HomePage_HUB_VenueProfileEditorModal from "../components/HomePage_HUB/HomePage_HUB_VenueProfileEditorModal.jsx";
...
async function findExistingPhotoDataByIdentity(identityLike, clubId = DEFAULT_CLUB_ID) {
  ...
  for (const id of ids) {
    try {
      ...
      console.error("[VenueEntryPage] Failed checking player photo:", err);
    }
  }
...
async function upsertPlayerFromMember(member, clubId = DEFAULT_CLUB_ID) {
  ...
  if (!displayName) {
    console.warn("[VenueEntryPage] upsertPlayerFromMember: empty name, skipping");
    return null;
  }
  ...
  try {
    ...
    console.error(
      "[VenueEntryPage] Failed to sync member into players collection:",
      err
    );
    return null;
  }
...
async function resolveSignedInRoleFromPlayerDoc(member, emailFromGoogle = "", clubId = DEFAULT_CLUB_ID) {
  ...
  for (const pid of candidateIds) {
    try {
      ...
      console.error("[VenueEntryPage] Failed reading player role:", err);
    }
  }
...
async function clearWithdrawnPlayerPrivateDetails({ clubId = DEFAULT_CLUB_ID, memberId = "", playerId = "" }) {
  ...
  if (playerId) {
    try {
      ...
      console.error("[VenueEntryPage] Could not clear player contact details:", err);
    }
  }
}

function buildVenueIdentity(venue) {
  const id = String(
    venue?.id || venue?.venueId || DEFAULT_CLUB_ID
  ).trim();

  const name = String(
    venue?.name || DEFAULT_CLUB_NAME
  ).trim();
...
export default function VenueEntryPage({
  identity,
  venue,
  ...
  const activeClub = useMemo(() => {
    const baseClub = venue || {
      ...
      logoUrl: "/favicon_nobackground.png",
      image: "",
      weeklyPlayTime: "",
    };
    ...
    return entryClubProfileOverride
      ...
      : baseClub;
  }, [venue, entryClubProfileOverride]);

  const activeClubIdentity = useMemo(
    () => buildVenueIdentity(activeClub),
    [activeClub]
  );
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      doc(db, "leagueVenues", activeClubId),
      (snap) => {
      ...
      (error) => {
        console.error("[VenueEntryPage] Could not load latest club profile:", error);
      }
    );
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "leagueVenues", activeClubId, "challengeNotices"),
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "leagueVenues", activeClubId, "incomingChallenges"),
      orderBy("createdAtMs", "desc"),
      limit(12)
    );
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        console.log(
          "[VenueEntryPage] Loaded members for",
          activeClubId,
          list.map((m) => ({
            id: m.id,
            fullName: m.fullName,
  ...
  const handleProfilePhotoChange = async (event) => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Could not prepare profile photo:", error);
  ...
  const handleSavePlayerProfile = async () => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Could not update profile:", error);
      ...
      try {
        ...
        if (playerId) {
          try {
            ...
            console.warn(
              "[VenueEntryPage] Identity audit could not read player profile:",
              error
            );
          }
        }
        ...
        async function countMatchingDocuments(collectionName, matches) {
          ...
          for (const [fieldName, fieldValue] of matches) {
            ...
            try {
              ...
              console.warn(
                `[VenueEntryPage] Identity audit could not query ${collectionName}.${fieldName}:`,
                error
              );
            }
          }
        ...
        console.error(
          "[VenueEntryPage] Identity Safety Audit failed:",
          error
        );
  ...
  const handleClubHeroPhotoChange = async (e) => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed updating club hero photo:", err);
  ...
  const handleNewPhotoChange = async (e) => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed preparing request photo:", err);
  ...
  const handleReminderPhotoChange = async (e) => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed preparing reminder photo:", err);
  ...
  const handleSaveReminderPhoto = async () => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed saving reminder photo:", err);
  ...
  const handleSaveWhatsAppReminder = async () => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed saving WhatsApp number:", err);
  ...
  const handleSubmitWithdrawalRequest = async () => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed to submit withdrawal request:", err);
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "leagueVenues", activeClubId, "fixtures"),
      orderBy("createdAtMs", "desc"),
      limit(8)
    );
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "venueLeagueFixtures", challengerChatFixture.fixtureId, "messages"),
  ...
  const handleSendChallengerChatMessage = async () => {
    ...
    try {
      await addDoc(collection(db, "venueLeagueFixtures", challengerChatFixture.fixtureId, "messages"), {
      ...
      console.error("[VenueEntryPage] Failed sending challenger chat message:", err);
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "leagueVenues", activeClubId, "chatMessages"),
      orderBy("createdAtMs", "asc"),
      limit(80)
    );
  ...
  const handleSendClubChatMessage = async () => {
    ...
    try {
      await addDoc(collection(db, "leagueVenues", activeClubId, "chatMessages"), {
        text,
        senderName,
        senderRole,
      ...
      console.error("[VenueEntryPage] Failed sending club chat message:", err);
  ...
  const handleSetClubAdminPrivilege = async (shouldBeAdmin) => {
    ...
    try {
      ...
      console.error(
        "[VenueEntryPage] Failed updating club administrator privilege:",
        error
      );
      setAdminPrivilegesError(
  ...
  const collectMatchingDocumentRefs = async ({
    ...
    for (const [fieldName, fieldValue] of matches) {
      ...
      try {
        ...
        console.warn(
          `[VenueEntryPage] Could not query ${collectionName}.${fieldName}:`,
          error
        );
      }
    }
  ...
  const handleTerminateClubMembership = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[VenueEntryPage] Could not queue squad cleanup:",
          cleanupStorageError
        );
      }
      ...
      console.error(
        "[VenueEntryPage] Failed terminating club membership:",
        error
      );
      setTerminationError(
  ...
  const handleCloseDepartureNotice = async (departureNotice) => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed acknowledging departure notice:", err);
  ...
  const markChallengeNoticeAcknowledged = async (noticePayload) => {
    ...
    await updateDoc(
      doc(db, "leagueVenues", activeClubId, "challengeNotices", noticeId),
      {
        status: "acknowledged",
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "venueLeagueFixtures", fixtureDiscussionModal.fixtureId, "messages"),
      orderBy("createdAtMs", "asc"),
      limit(80)
    );
  ...
  const handleSendFixtureDiscussionMessage = async () => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed sending fixture discussion message:", err);
  ...
  const handleAcceptFixtureAlternative = async (noticePayload) => {
    ...
    try {
      ...
      batch.set(doc(db, "venueLeagueFixtures", fixtureId), patch, { merge: true });

      participatingClubIds.forEach((clubId) => {
        batch.set(doc(db, "leagueVenues", clubId, "fixtures", fixtureId), patch, { merge: true });
      });

      await batch.commit();
      await markChallengeNoticeAcknowledged(noticePayload);
    } catch (err) {
      console.error("[VenueEntryPage] Failed accepting fixture alternative:", err);
  ...
  const handleContinueOriginalFixtureRequest = async (noticePayload) => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed continuing fixture request:", err);
  ...
  const handleAcceptFixtureChange = async (noticePayload) => {
    ...
    try {
      ...
      console.error("[VenueEntryPage] Failed accepting fixture change:", err);
  ...
  const handleSubmitFixtureAlternative = async () => {
    ...
    try {
      ...
      if (targetClubId) {
        await addDoc(collection(db, "leagueVenues", targetClubId, "challengeNotices"), {
          noticeId,
          type: "challenge_change_reply",
      ...
      console.error("[VenueEntryPage] Failed sending alternative fixture suggestion:", err);
    ...
    if (notice.type === "challenge_cancelled") {
      ...
      if (noticeId) {
        try {
          ...
          console.error("[VenueEntryPage] Failed acknowledging challenge notice:", err);
  ...
  const handleAcceptChallenge = async (challenge) => {
    ...
    try {
      ...
      batch.set(
        doc(db, "venueLeagueFixtures", fixtureId),
        fixturePayload,
        { merge: true }
      );
      ...
      participatingClubIds.forEach((clubId) => {
        batch.set(
          doc(db, "leagueVenues", clubId, "fixtures", fixtureId),
          fixturePayload,
          { merge: true }
        );
      });
      ...
      console.error("[VenueEntryPage] Failed accepting challenge:", err);
  ...
  const handleRejectChallenge = async (challenge) => {
    ...
    try {
      await updateDoc(
        doc(db, "leagueVenues", activeClubId, "incomingChallenges", challenge.challengeId),
        {
          status: "rejected",
          respondedAt: serverTimestamp(),
          respondedAtMs: Date.now(),
      ...
      console.error("[VenueEntryPage] Failed rejecting challenge:", err);
  ...
  return (
    <div className="page entry-page">
      ...
      <HomePage_HUB_VenueProfileEditorModal
        ...
        onSaved={(updatedClub) => {
          ...
          if (typeof onVenueUpdated === "function") {
            onVenueUpdated(updatedClub);
          }
