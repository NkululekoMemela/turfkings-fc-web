/*
 * VENUE LEAGUE UI CLONE
 *
 * Visual structure copied from the proven Club component.
 * All data access must remain beneath Venue League paths.
 */
// src/components/ClubChat/VenueLeagueChatWidget.jsx
import React, { useCallback, useEffect, useRef, useState } from "react";
import { db } from "../../firebaseConfig";
import {
  getVenueParticipantLogosCollection,
  loadVenueArchivedHighlights,
  loadVenueRawHighlights,
...
export function VenueLeagueChatWidget({
  activeClubId,
  activeClubName,
  currentUser,
  selectedMember,
    ...
    async function loadClubChatPhotos() {
      ...
      try {
        const snap = await getDocs(
          getVenueParticipantLogosCollection(db, activeClubId)
        );
        ...
        console.warn(
          "[VenueLeagueChatWidget] Could not load player photos:",
          err
        );
      }
    }
  ...
  const toggleClubChatReaction = async (message, emoji) => {
    ...
    try {
      await updateDoc(doc(db, "leagueVenues", activeClubId, "chatMessages", message.id), {
        [`reactionsByUser.${key}`]: nextEmoji,
      });
      setActiveReactionPickerMessageId(null);
    } catch (err) {
      console.error("[VenueLeagueChatWidget] Failed updating reaction:", err);
  ...
  useEffect(() => {
    try {
      setClubChatLastSeenMs(
        Number(window.localStorage.getItem(`fanm_venue_league_chat_seen_${activeClubId}`) || 0)
      );
    } catch {
  ...
  const loadChatHighlights = async () => {
    ...
    try {
      const [raw, archived] = await Promise.all([
        loadVenueRawHighlights(activeClubId, matchId),
        loadVenueArchivedHighlights(activeClubId, matchId),
      ]);
      ...
      console.error("[VenueLeagueChatWidget] Failed loading highlights for chat attach:", err);
  ...
  const handleDeleteClubChatMessage = async (message) => {
    ...
    try {
      await deleteDoc(doc(db, "leagueVenues", activeClubId, "chatMessages", message.id));
    } catch (err) {
      console.error("[VenueLeagueChatWidget] Failed deleting chat message:", err);
  ...
  const handleSendChallengerChatMessage = async () => {
    ...
    try {
      ...
      console.error("[VenueLeagueChatWidget] Failed sending challenger chat message:", err);
  ...
  const handleSendClubChatMessage = async () => {
    ...
    try {
      ...
      console.error("[VenueLeagueChatWidget] Failed sending club chat message:", err);
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(
        `fanm_venue_league_chat_seen_${activeClubId}`,
